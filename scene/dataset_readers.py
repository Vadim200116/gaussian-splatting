...
def readColmapCameras(cam_extrinsics, cam_intrinsics, depths_params, images_folder, depths_folder, semantics_folder, masks_folder, test_cam_names_list):
    cam_infos = []

    train_img_names = [cam_extrinsics[key].name for key in cam_extrinsics if cam_extrinsics[key].name not in test_cam_names_list]
    img_name2mask_name_mapping = {img_name: idx for idx, img_name in enumerate(natsorted(train_img_names))}
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        mask_path = os.path.join(masks_folder, f"{img_name2mask_name_mapping[image_name]}.png") if masks_folder and image_name not in test_cam_names_list != "" else ""
        semantics_path = os.path.join(semantics_folder, f"{extr.name[:-n_remove]}.npy") if semantics_folder != "" else ""

        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, depth_params=depth_params,
                              image_path=image_path, image_name=image_name, depth_path=depth_path,
                              width=width, height=height, is_test=image_name in test_cam_names_list,
                              semantics_path=semantics_path, mask_path=mask_path)
...
def readColmapSceneInfo(path, images, depths, masks, eval, train_test_exp, load_semantics=False, llffhold=8):
    ...
    depth_params_file = os.path.join(path, sparse_dir, "depth_params.json")
    ...
    cam_infos_unsorted = readColmapCameras(
        ...
        semantics_folder= os.path.join(path, "SD") if load_semantics else "",
        masks_folder = masks,
